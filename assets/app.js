...
els.btnFormat.addEventListener("click", async () => {
  ...
  const CRAFT_BLOCKS = {
    ...
    "Short Advert": `SHORT ADVERT — CRAFT INSTRUCTIONS (award-winning copywriter framework, applies only to Short Advert, no other production type): write as an award-winning creative advertising copywriter would, emotionally compelling, not a travelogue, not a product label, not a diary narration of someone's day. Follow these rules across the ad:
1. Begin by creating a vivid scene the target customer wants to experience.
2. Focus first on emotion and lifestyle, not the product.
3. Build curiosity before mentioning the product by name.
4. Introduce the product as the natural solution or companion to the experience already built, never a random insert.
5. Highlight transformation and feeling over features and specs.
6. Appeal to the senses, sight, sound, touch, smell, taste, wherever it genuinely fits the scene.
7. End with a memorable emotional payoff and a strong, explicit call to action.
8. Use cinematic language that is easy to visualize on screen.
9. Every line should move the audience closer to wanting the product, nothing wasted on filler.
10. Leave the audience remembering the feeling more than just the product's name.

STRUCTURE: Hook, Emotional Scene, Emotional Build, Product Reveal, Emotional Benefit, Memorable Closing, Call to Action. This ad is exactly ${n} segment${n > 1 ? "s" : ""} long. ${n > 1 ? `Spread these beats across the ${n} segments in that order: earlier segments carry the Hook, Emotional Scene, and Emotional Build (curiosity, feeling, lifestyle, no product name spoken yet), a middle-to-later segment carries the Product Reveal (the product is introduced by name as the natural answer to the feeling already built), and the final segment, Segment ${n}, use the "On-Camera + Brand Close" Type for it, carries the Emotional Benefit, Memorable Closing, and an explicit spoken call to action.` : `Since this is a single segment, compress the full arc, hook, feeling, product reveal, and call to action, into this one segment's word-count budget, it has no later segment to defer any beat to.`}

OVERRIDE OF THE SHARED BASE RULE ON BRAND TIMING: the general rule elsewhere requiring the brand to be named starting in segment 1 does not apply here, delaying the product's name until its Product Reveal beat is the intended technique for Short Advert, not an omission. The product must still be visually present somewhere in the ad, for example visible in an early Text-to-Image Prompt, even before it is named aloud, so it never disappears from the ad entirely, and it must be explicitly named by its Product Reveal segment and again in the closing call to action, never held back for the entire ad.

CRITICAL DISCIPLINE, none of these are optional: never speak the product's name out loud in a segment before its designated Product Reveal segment, a Hook, Emotional Scene, or Emotional Build segment that names the product out loud has missed its job, the product stays implied through visual presence only until its reveal beat. Never end the ad, including a single-segment ad's one and only segment, without an explicit spoken call to action landed in the TTS Script, a warm closing feeling with no invitation to act is not a Memorable Closing, it is an incomplete ad and has failed the assignment. The Type field on every segment must be copied exactly, character for character, from the three allowed values given earlier, On-Camera, Voiceover + B-Roll, or On-Camera + Brand Close, never a blended or invented label like "On-Camera + B-Roll".

THE TTS SCRIPT MUST ALWAYS BE AN ACTUAL SENTENCE, NEVER A LIST OF PHRASES: a comma-separated string of disconnected noun phrases or mood-board keywords is not a line, do not write it that way under any circumstance, even when trying to stay concise or hit a word count. A broken list like "warm sun, vibrant streets, some rhythm, sweet moments" is not something a real person would ever say out loud, it is a caption broken into fragments, and it is never acceptable, reject that shape entirely. Write a genuine sentence a real person would speak instead, built around a real subject and verb, using close to the full word-count budget.

THIS LINE MUST BE ORIGINAL AND SPECIFIC TO THIS EXACT SCRIPT, NEVER GENERIC OR REUSED: ground every TTS Script in a concrete, specific detail that belongs only to this segment's own scene, an actual object, action, or sensory detail drawn from what the Text-to-Image Prompt for that same segment describes, not a vague mood phrase that could be pasted into any ad for any product in any city. If any instruction elsewhere in this prompt contains a sample sentence written to illustrate a rule, that sample exists only to show sentence shape and rhythm, copying it, or any close paraphrase of it, into the actual output is strictly forbidden and counts as a failure to write original ad copy, always write a brand new sentence about this script's own product, scene, and details instead.

Every Image-to-Video Prompt must still give the subject real physical energy and kinetic specificity, genuine movement with momentum, never a passive static beat. If the Type is On-Camera or On-Camera + Brand Close, the subject speaks the relevant line aloud to camera, per the shared On-Camera rule, and any product-handling action (drinking, opening, holding up) must be staged as its own beat before or after the spoken line, never simultaneous with it, since a person cannot speak clearly with their mouth on the product. Stay inside the real word-count target for each segment's duration, roughly 22 to 28 words for a 10-second segment, roughly 33 to 42 words for a 15-second segment (rule 6), vivid cinematic language still has to fit inside that budget, cut an adjective before ever running over it, and a bare fragment well under the target fails the assignment just as badly as running over it.\n\n`,
