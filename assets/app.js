...
Every Image-to-Video Prompt must still give the subject real physical energy and kinetic specificity, genuine movement with momentum, never a passive static beat. If the Type is On-Camera or On-Camera + Brand Close, the subject speaks the relevant line aloud to camera, per the shared On-Camera rule, and any product-handling action (drinking, opening, holding up) must be staged as its own beat before or after the spoken line, never simultaneous with it, since a person cannot speak clearly with their mouth on the product. Use close to the full word-count budget available for each segment's duration (see rule 6), a bare fragment is never enough.\n\n`,
