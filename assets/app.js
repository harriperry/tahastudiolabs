...
THE TTS SCRIPT MUST ALWAYS BE AN ACTUAL SENTENCE, NEVER A LIST OF PHRASES: a comma-separated string of disconnected noun phrases or mood-board keywords is not a line, do not write it that way under any circumstance, even when trying to stay concise or hit a word count. For example "Warm sun, vibrant streets, Douala's rhythm, sweet moments" is not something a real person would ever say out loud, it is a caption broken into fragments, and it is never acceptable, reject that shape entirely. Instead write a genuine sentence a real person would speak, built around a real subject and verb, for example "The sun's warm here, the streets never stop moving, and this rhythm gets under your skin" is an actual line, notice it uses close to the full word-count budget while still sounding spoken. Every TTS Script in this ad must pass this test: read it out loud, if it sounds like a list of tags instead of a person talking, rewrite it as a real sentence before outputting it.
